function collapsableForm() {
  ...
  const signUpCollapseBtn = document.getElementById("collapsable-btn");
    ...
    el.addEventListener("show.bs.collapse", function () {
      collapseElements.forEach((colEl) => {
        if (el !== colEl && colEl.classList.contains("show")) {
          var collapseInstance = bootstrap.Collapse.getInstance(colEl);
          collapseInstance.hide();
        }
      });

      if (el === document.querySelector(logInCollapseBtn.dataset.bsTarget)) {
